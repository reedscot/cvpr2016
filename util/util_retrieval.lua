...
function attributes_at_k(txt_dir, img_dir, cls_list, kvals, scorefunc, limit, attfunc)
    local fea_img = {}
    local fea_txt = {}
    ...
    scores = torch.zeros(#fea_txt, total_img)
    matches = torch.zeros(#fea_txt, total_img)
    cosine = torch.zeros(#fea_txt, total_img)
    local min_score = math.huge
    local max_score = -math.huge
    for i = 1,#fea_txt do
        local res = rank_images(fea_txt[i], i, fea_img, scorefunc, attfunc)
        scores[{i,{}}] = res[1]
        matches[{i,{}}] = res[2]
        cosine[{i,{}}] = res[3]
    end
    local results = {}
    for _,k in ipairs(kvals) do
        results[k] = torch.mean(cosine:narrow(2,1,k))
    end
    return results
end
