...
function model_utils.combine_all_parameters(...)
    ...
    for i = 1, #networks do
        if networks[i]['weights'] ~= nil then
            parameters[#parameters + 1] = networks[i].weights
            gradParameters[#gradParameters + 1] = networks[i].grads
        else
...
function model_utils.split(inputstr, sep)
    if sep == nil then
        sep = "%s"
    end
    local t = {}; 
